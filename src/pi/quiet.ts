 ...
 *
 * dev-house's extension also loads inside the subagent processes it spawns
...
/** True inside a dev-house subagent process (marked in `spawnPiProcess`). */
export function isSubagentProcess(): boolean {
  return process.env.DEV_HOUSE_SUBAGENT === "1";
}
