...
test("knowledge action records Master-approved knowledge", async () => {
  ...
  assert.match(readFileOr(join(knowledgeDir(join(deps.root, ".pi", "dev-house"), "backend"), "knowledge.md")), /Pagination belongs/);
});
...
test("agent workers have no knowledge write action of their own", async () => {
  ...
  assert.match(readFileOr(join(knowledgeDir(join(deps.root, ".pi", "dev-house"), "backend"), "engineering-standards.md")), /Always validate query params/);
});
...
test("completion flushes the task's decisions into the knowledge store", async () => {
  ...
  assert.equal(result.ok, true, result.message);
  const decisions = readFileOr(join(knowledgeDir(join(deps.root, ".pi", "dev-house"), "backend"), "decisions.md"));
